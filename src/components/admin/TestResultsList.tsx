...
import { ChevronDown, ChevronRight, Clock, Coins, CheckCircle, XCircle, AlertCircle } from "lucide-react";
...
export default function TestResultsList({ expectedResult, results }: TestResultsListProps) {
    ...
    return (
        <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <div className="border rounded-md bg-neutral-50 dark:bg-neutral-900/50">
                ...
                >
                    <span className="flex items-center gap-2">
                        {isExpectedResultOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
            ...
            <div className="space-y-2">
                <h4 className="text-xs font-semibold uppercase text-gray-500 tracking-wider mb-2">Latest Model Executions</h4>
                {results.length === 0 ? (
                    ...
                    results.map((result) => (
                        <div key={result.model} className="border rounded-md overflow-hidden bg-white dark:bg-neutral-950">
                            {/* Result Header */}
                            <div
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    {openResults[result.model] ? <ChevronDown className="w-4 h-4 text-gray-400" /> : <ChevronRight className="w-4 h-4 text-gray-400" />}
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    {/* Score */}
                                    {result.score !== null && (
                                        <div className={cn("px-2 py-0.5 rounded text-xs font-semibold border", getScoreColor(result.score))}>
                                    ...
                                    <div className="flex items-center gap-3 text-xs text-gray-500 border-l pl-3 ml-1 hidden md:flex">
                                        <span className="flex items-center gap-1" title="Token Cost">
                                    ...
                                    <div className="p-4 space-y-4 bg-neutral-50/50 dark:bg-neutral-900/20">
                                        ...
                                        <div className="flex gap-4 text-xs text-gray-500 md:hidden pb-2 border-b">
                                            <span className="flex items-center gap-1">
                                                <Coins className="w-3 h-3" /> Cost: {formatCost(result.tokens_cost)}
                                            </span>
                                            <span className="flex items-center gap-1">
                                                <Clock className="w-3 h-3" /> Time: {formatTime(result.execution_time_ms)}
                                            </span>
                                        </div>

                                        {/* Output */}
                                        <div>
                                            <h5 className="text-xs font-semibold text-gray-500 mb-1">Model Output</h5>
                                            <div className="bg-white dark:bg-black border rounded p-3 text-sm">
                                                {result.output ? (
                                                    <div className="whitespace-pre-wrap">{result.output}</div>
                                                ) : (
                                                    <span className="text-gray-400 italic">No output available</span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {result.explanation && (
                                            <div>
                                                <h5 className="text-xs font-semibold text-gray-500 mb-1">Evaluation Explanation</h5>
